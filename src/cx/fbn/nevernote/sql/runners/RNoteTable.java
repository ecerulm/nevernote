...
public class RNoteTable {
	private final ApplicationLogger 		logger;
	public final RNoteTagsTable				noteTagsTable;
	public RNoteResourceTable				noteResourceTable;
	...
	// Constructor
	public RNoteTable(ApplicationLogger l, RDatabaseConnection d) {
		logger = l;
		db = d;
		id = 0;
		noteResourceTable = new RNoteResourceTable(logger, db);
		noteTagsTable = new RNoteTagsTable(logger, db);
...
//		indfm = new SimpleDateFormat("EEE MMM dd HH:mm:ss yyyy");
		...
		if (loadTags) {
			...
			RTagTable tagTable = new RTagTable(logger, db);
			for (int i=0; i<n.getTagGuids().size(); i++) {
		...
		if (loadContent) {
			...
			//n.setContent(query.value(16).toString());
	...
	public void updateNoteNotebook(String guid, String notebookGuid, boolean expungeFromRemote) {
		...
		RNotebookTable notebookTable = new RNotebookTable(logger, db);
		RDeletedTable deletedTable = new RDeletedTable(logger, db);
...
//		query.prepare("Update Note set deleted=0, active=true, isDirty=true where guid=:guid");
	...
	public List <Note> getDirty() {
		...
		check = query.exec("Select guid from Note where isDirty = true and notebookGuid not in (select guid from notebook where local = true)");
		if (!check) 
